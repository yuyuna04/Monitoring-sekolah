...
import { FiMail, FiLock, FiEye, FiEyeOff, FiUser, FiKey, FiCalendar } from 'react-icons/fi';
...
export default function Login() {
  ...
  // Mode & tipe
  const [mode, setMode]           = useState('login');   // login | daftar | lupa
  ...
  // ── Fetch kelas berdasarkan tahun ajaran ────────────────────────────────────
  useEffect(() => {
  ...
  const fetchKelas = async () => {
    let query = supabase.from('kelas').select('id, nama_kelas, tahun_ajaran_id').order('nama_kelas');
    ...
    try {
      ...
      if (data.user) {
        ...
        if (tipeAkun === 'orangtua' && siswa) {
          ...
          // Hubungkan guru ke kelas sebagai wali kelas
          await supabase.from('kelas')
            .update({ wali_kelas_id: data.user.id })
            .eq('id', kelasDipilih);
    ...
    <div className="min-h-screen flex flex-col bg-white" style={{ maxWidth: '480px', margin: '0 auto' }}>
      ...
      <div className="relative overflow-hidden" style={{
        ...
        <div style={{ textAlign: 'center', paddingTop: '60px', position: 'relative', zIndex: 10 }}>
          ...
          {tahunAjaranList.find(t => t.id === tahunAjaranAktif) && (
            <div style={{
              ...
              📅 TA {tahunAjaranList.find(t => t.id === tahunAjaranAktif)?.nama}
            </div>
          )}
        </div>
      ...
      <div style={{
            ...
            {['login', 'daftar'].map(m => (
              <button key={m} type="button"
                ...
                style={{
                  ...
                  background: mode === m ? '#CC0000' : 'transparent',
                  color:      mode === m ? 'white' : '#666',
                  boxShadow:  mode === m ? '0 2px 8px rgba(204,0,0,0.4)' : 'none'
                }}>
        ...
        {mode === 'daftar' && (
          <form onSubmit={handleDaftar}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                {[
                  ...
                  <button key={t.val} type="button"
                    onClick={() => { setTipeAkun(t.val); setKodeAkses(''); setKelasDipilih(''); setError(''); }}
                    style={{
              ...
              <>
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <input type="text" value={nisAnak} onChange={e => setNisAnak(e.target.value)}
                    placeholder="Contoh: 2024001" required style={inputStyleNoIcon} />
                  <p style={{ fontSize: '11px', color: '#999', marginTop: '4px' }}>
                    NIS bisa dilihat di rapor atau tanya wali kelas
                  </p>
                </div>
                <div style={{ marginBottom: '16px' }}>
              ...
              <>
                ...
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <select value={tahunAjaranAktif}
                      onChange={e => { setTahunAjaranAktif(e.target.value); setKelasDipilih(''); }}
                      style={inputStyle}>
                      <option value="">-- Pilih Tahun Ajaran --</option>
                      {tahunAjaranList.map(t => (
                        <option key={t.id} value={t.id}>
                          TA {t.nama} {t.aktif ? '(Aktif)' : ''}
                        </option>
                      ))}
                    </select>
                  </div>
                ...
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <select value={kelasDipilih} onChange={e => setKelasDipilih(e.target.value)}
                    required style={inputStyleNoIcon}>
                  ...
                  {daftarKelas.length === 0 && (
                    <p style={{ fontSize: '11px', color: '#CC0000', marginTop: '4px' }}>
                      Belum ada kelas tersedia untuk tahun ajaran ini. Hubungi admin.
                    </p>
                  )}
                </div>
              </>
            ...
            {/* Info box */}
            <div style={{
              background: '#EBF5FB', border: '1px solid #BDE0F5',
              borderRadius: '12px', padding: '10px 12px',
              marginBottom: '16px', fontSize: '12px', color: '#1565C0'
            }}>
              {tipeAkun === 'orangtua' && 'ℹ️ Akun akan otomatis terhubung ke data anak berdasarkan NIS. Pastikan NIS yang dimasukkan sesuai dengan NIS di rapor anak.'}
          ...
          <form onSubmit={handleLupa}>
            ...
            <button type="button" onClick={() => { setMode('login'); setError(''); setSuccess(''); }}
              style={{
                ...
                border: '2px solid #E0E0E0', borderRadius: '14px',
